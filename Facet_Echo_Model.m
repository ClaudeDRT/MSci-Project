function [P_t_full,P_t_ml,P_t_full_comp,P_t_ml_comp] = Facet_Echo_Model(lambda,bandwidth,P_T,h,v,pitch,roll,prf,G_0,D_0,N_b,t,PosT,surface_type,sigma_0_snow_surf,sigma_0_snow_vol,kappa_e,tau_snow,c_s,h_s,sigma_0_ice_surf,sigma_0_lead_surf,sigma_0_mp_surf)
...
% Based on theory detailed in Makynen et al 2009, Giles et al 2007,
% Wingham et al 2006, Ulaby et al 2014
...
% figure;trisurf(TRI,PosTx,PosTy,PosTz,'edgecolor','none')

% Construct hamming window (cannot use for simplified surfaces)
dx = max(diff(PosT(:,1)));
H = bsxfun(@times, ones(range(PosT(:,2))/dx+1,1), hamming(range(PosT(:,1))/dx+1)'); H=H(TRI(:,1));
...
parfor i = 1:length(m)
    ...
    % Apply hamming weighting to synthetic beam pattern
    P_m = H.*P_m;
    
    %% Compute transmitted power envelope
    
    % Time offset
    % tc = 2*(sqrt(sqrt(x_0^2 + y_0^2).^2*(1 + h/Re)+h^2)-h)/c; % slant-range time correction
...
%     mu_t(SURFACE_TYPE==1) = -(rho_i_0*alpha)/2*(1 + alpha*sin(theta_pr(SURFACE_TYPE==1)).^2).^(-3/2); % Kurtz et al 2014 function
    ...
    % Pulse-limited echo (for conventional single-look echo use only m=0)
    % P_r = ((lambda^2*P_T)/(4*pi)^3)*(0.5*c*h)*bsxfun(@times, bsxfun(@rdivide, sigma_0_P_t, R.^4), G.^2.*A_facets);
    
    echo_t = nansum(P_r,1);
    
    % Keeps memeory use low during loop
    T = []; P_t = []; P_r = []; theta_PR = []; sigma_0_P_t = [];
    
    % Single-look echo stack
    P_t_full(i,:) = real(echo_t);
