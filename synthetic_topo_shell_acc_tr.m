function [PosT,surface_type] = synthetic_topo_shell_acc_tr(op_mode,topo_type,sigma_surf,l_surf,H_surf,dx,L_w,L_h,D_off_acc_tr,f_p,L_ang)
...
if L_w>0
    if  L_ang ==90
        L_x_min_steps = max([round(L/(2*dx)-(D_off_acc_tr/dx)-L_w/(2*dx)) 1]);
        L_x_max_steps = min([round(L/(2*dx)-(D_off_acc_tr/dx)+L_w/(2*dx))-1 (L/dx)]);
        z(L_x_min_steps:L_x_max_steps,:) = -L_h;
        surface_type(L_x_min_steps:L_x_max_steps,:) = 0;
    elseif L_ang ==0
        L_y_min_steps = max([round(W/(dx*2)-D_off_acc_tr/dx-L_w/(2*dx)) 1]);
        L_y_max_steps = min([round(W/(dx*2)-D_off_acc_tr/dx+L_w/(2*dx))-1 (W/dx)]);
        z(:,L_y_min_steps:L_y_max_steps) = -L_h;
        surface_type(:,L_y_min_steps:L_y_max_steps) = 0;
    elseif L_ang<=45
        a=(round(L_w/(2*dx*sin(L_ang*pi/180))));
        for xi = -a:(L/dx)+a
            line_y = round((W/(2*dx)) + tan(L_ang*pi/180)*(xi - ((L/(2*dx)) - D_off_acc_tr/dx)));
            for yi = 1:(W/dx)
                if yi==line_y
    ...
    elseif L_ang>45 & L_ang<90
        ...
        for yi = -b:(W/dx)+b
            line_x = round(((L/(2*dx)) - D_off_acc_tr/dx) + (yi - (W/(2*dx)))/tan(L_ang*pi/180));
            for xi = 1:(L/dx)
                if xi==line_x
