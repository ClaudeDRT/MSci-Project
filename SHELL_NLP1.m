...
sigma_si = [0.001:0.0005:0.004]; % sea ice rms height (default = 0.002 m)
...
sigma_surf = [0:0.05:0.5]; % large-scale rms roughness height (default = 0.1 m)
l_surf = [0:1:10]; % large-scale correlation length (default = 5 m)
...
L_ang = 0; %lead angle from the across-track direction (between 0° and 90°)
    ...
    for j = 1:length(vec2)
        ...
        for k = 1:length(vec3)
            ...
            for a = 1:length(vec4)
                ...
                save(['./FEM_Sim_NLP1_temp'],'t','P_t_ml_range','parameters_lookup');%,'-append');
                if match(4)>0
                    counter = 0;
                else
                end
...
save('FEM_Sim_NLP1','t','parameters_lookup','P_t_ml_range','-append');
